...
std::string makeEvent(const std::string& action, const std::string& detail) {
    std::string out = "\x1e";
    out += "\"action\":";
...
void Logger::event(const std::string& action, const std::string& detail,
                   spdlog::level::level_enum level) {
    get()->log(level, makeEvent(action, detail));
}
